...
public class OracleEmbeddingStoreFactory {
    ...
    public OracleEmbeddingStore create() {
        ...
        configuration.getString("data-source").ifPresent(bn -> builder.dataSource(BeanResolver.resolve(DataSource.class, bn)));
        configuration.getBoolean("embedding-table").ifPresent(b -> {
                if (b) {
                    builder.embeddingTable(creatembeddingTable());
                }
            }
        );
        configuration.getBoolean("exact-search").ifPresent(builder::exactSearch);
        configuration.getString("vector-index").ifPresent(v -> builder.vectorIndex(CreateOption.valueOf(v)));
    ...
    private EmbeddingTable creatembeddingTable() {
        ...
        configuration.getString("create-option").ifPresent(v -> builder.createOption(CreateOption.valueOf(v)));
        configuration.getString("name").ifPresent(builder::name);
        configuration.getString("id-column").ifPresent(builder::idColumn);
        configuration.getString("embedding-column").ifPresent(builder::embeddingColumn);
        configuration.getString("text-column").ifPresent(builder::textColumn);
        configuration.getString("meta-column").ifPresent(builder::metadataColumn);
